...
- Nowadays I use WSL2 extensively with the excelent windows terminal.
...
- *[optional]* install [[https://github.com/eza-community/eza][eza]] for enhanced directory listings (=sudo apt install eza= on Ubuntu; =brew install eza= on OSx). The aliases fall back to =ls= if it is unavailable.
